...
const generateWAMessageContent = async (message, options) => {
    ...
    if ('text' in message) {
        ...
        // WhatsApp has shipped pollCreationMessage V2 through V6 (newer
        // client versions increasingly expect these instead of the legacy
        // V1 field). Default stays V1 for backwards compatibility with
        // existing bots; pass `pollVersion: 2..6` to opt into a newer one.
        const POLL_FIELD_BY_VERSION = {
            1: 'pollCreationMessage',
            2: 'pollCreationMessageV2',
            3: 'pollCreationMessageV3',
            4: 'pollCreationMessageV4',
            5: 'pollCreationMessageV5',
            6: 'pollCreationMessageV6',
        };
        const pollField = POLL_FIELD_BY_VERSION[message.poll.pollVersion || message.poll.version || 1]
            || 'pollCreationMessage';
        m[pollField] = pollPayload;
    }
    else if ('pollAddOption' in message) {
        // Adds an option to an existing poll (WA proto field 121). Caller
        // must supply the poll's messageSecret / encKey used at creation
        // time, plus the option name(s) to add.
        m.messageContextInfo = {
            messageSecret: message.pollAddOption.messageSecret || (0, crypto_1.randomBytes)(32),
        };
        const names = Array.isArray(message.pollAddOption.values)
            ? message.pollAddOption.values
            : [message.pollAddOption.name].filter(Boolean);
        m.pollAddOptionMessage = {
            pollOption: names.map(optionName => ({ optionName })),
        };
    }
    else if ('liveLocation' in message) {
        m.liveLocationMessage = {
            location: {
                degreesLatitude: message.liveLocation.degreesLatitude,
                degreesLongitude: message.liveLocation.degreesLongitude,
            },
            accuracyInMeters: message.liveLocation.accuracyInMeters,
            speedInMps: message.liveLocation.speedInMps,
            degreesClockwiseFromMagneticNorth: message.liveLocation.degreesClockwiseFromMagneticNorth,
            caption: message.liveLocation.caption ? { text: message.liveLocation.caption } : undefined,
            sequenceNumber: message.liveLocation.sequenceNumber || Date.now(),
        };
    }
    else if ('groupInvite' in message) {
        // "You've been invited to join a group" style message.
        m.groupInviteMessage = {
            groupJid: message.groupInvite.groupJid,
            ...
            caption: message.groupInvite.caption ? { text: message.groupInvite.caption } : undefined,
        };
    }
    else if ('eventInvite' in message) {
        m.eventInviteMessage = {
            eventId: message.eventInvite.eventId,
            eventTitle: message.eventInvite.eventTitle,
            jpegThumbnail: message.eventInvite.jpegThumbnail,
            startTime: message.eventInvite.startTime,
            endTime: message.eventInvite.endTime,
            caption: message.eventInvite.caption,
            isCanceled: message.eventInvite.isCanceled || false,
            callLink: message.eventInvite.callLink,
        };
    }
    ...
    if ('spoiler' in message && !!message.spoiler) {
        // spoilerMessage (proto field 118) is a FutureProofMessage wrapper,
        // same shape as viewOnceMessage — it just wraps the real content
        // and tells the client to render it blurred until tapped.
...
const normalizeMessageContent = (content) => {
    ...
    function getFutureProofMessage(message) {
        return ((message === null || message === void 0 ? void 0 : message.ephemeralMessage)
            ...
            || (message === null || message === void 0 ? void 0 : message.spoilerMessage)
