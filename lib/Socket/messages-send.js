...
// --- Missing from this fork's Utils build; local implementations so the module can load. ---
// Generates an iOS-style message ID (20 uppercase hex chars) for when config.browser signals iOS/Safari.
const generateIOSMessageID = () => randomBytes(10).toString('hex').toUpperCase();
// Participant-list hash ("phash") WhatsApp uses server-side to detect stale device lists on group sends.
const generateParticipantHashV2 = (participants) => {
    const sorted = [...participants].sort();
    const hash = createHash('sha256').update(sorted.join(',')).digest('base64').slice(0, 6);
    return `2:${hash}`;
};
// Local cache for message secrets (used by WA's bot/reaction e2e feature). No reader exists
// elsewhere in this fork yet — this just stores it so callers relying on it later don't break.
const botMessageSecrets = new Map();
const setBotMessageSecret = (msgId, secret, jid) => {
    botMessageSecrets.set(msgId, { secret, jid, ts: Date.now() });
};
